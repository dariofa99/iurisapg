...
$(document).ready(function(e) {
  ...
  $("#myModal_videollamadas").on("click",'#newtab-stream-cases',function(e){
  ...
  $("#myModal_videollamadas").on("click",'#ask-stream-cases',function(e){
    ...
    var route = "/expediente/sharestream/"+$('#ask-stream-cases').attr('data-id');
    $.ajax({
      url: route,
      type: 'GET',
      datatype: 'json',
...
$("#tipo_busqueda").change(function(){
  ...
  $("#btnActualizar").click(function(){
    ...
    //errors = validateForm('form_expediente_user_edit');
    if (expediente_id !='' && expidnumberest!=''  && expramaderecho_id!=''  && expestado!='' &&  exptipoproce_id !='') {
   // var token = $("#token").val();  
    ...
    type:'POST',
    datatype: 'json',
    data:{'expidnumberest':expidnumberest,'expramaderecho_id':expramaderecho_id,'expestado':expestado,'exptipoproce_id':exptipoproce_id,'oldexpidnumberest':oldexpidnumberest},
    cache: false,
    beforeSend: function(xhr){
      $("#wait").css("display", "block"); 
      xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));  
...
$("#table_list_est_asig_doc").on('click','.btn_deleteasignacionaoc',function(e){
  e.preventDefault();
...
if (n != -1) {
  ...
  $("#btn_toggle_active").removeClass('switch-on').addClass('switch-off');
});
}else{
 $("#tbl_users input[type='text']").each(function(){
  $(this).val(1);
  $("#btn_toggle_active").removeClass('switch-off').addClass('switch-on');
...
$("#actestado").on("change",function(){
  ...
  if ($(this).val()=='104') {
    ...
    $("#myform_act_edit_docente #fecha_limit_doc").prop('disabled',true)
    }else{
    hideElement('addNotasAct','class');
   $(".addNotasAct .required").prop('disabled',true);
   $("#myform_act_edit_docente #fecha_limit_doc").prop('disabled',false)
...
$("#switch_act_fc").on('click',function(){ 
 if($(this).attr('dataset')=='1'){
   msj = confirm('¿Esta seguro de desactivar el cierre de corte?\nRecuerde que antes debe cerrar el corte.')
...
$("#tbl_ajax").on('click','.btn_change_status',function(){
  ...
  if (estado == 139) {
    var msj = alertify.confirm("¿Esta seguro de cambiar el estado?\nSe eliminaran las notas");
  ...
  $(".btn_new_act").on("click",function(){
    ...
    $("#myformCreateAct input[name=fecha_limit]").prop('disabled',false).show();
    if($(this).attr('id')=='btn_new_anex'){
    $("#actestado_id").val(136);
    $("#myformCreateAct input[name=fecha_limit]").prop('disabled',true).hide();
...
$("#myFormNotasSearchInd select[name=periodo_id]").on('change',function(){
  var periodo_id = $(this).val();
  if (periodo_id!="") {
...
$("#myFormNotasSearch input[name=type_repor]").on("change",function(e){
  ...
  if($(this).val()=='periodo'){
    $("#myFormNotasSearch select[name=segmento_id]").prop('disabled',true);
  }else{
    $("#myFormNotasSearch select[name=segmento_id]").prop('disabled',false);
  }
});

$("#myFormNotasSearchInd input[name=type_repor]").on("change",function(e){
  console.log($(this).val());
  if($(this).val()=='periodo'){
    $("#myFormNotasSearchInd select[name=segmento_id]").prop('disabled',true);
  }else{
    $("#myFormNotasSearchInd select[name=segmento_id]").prop('disabled',false);
  }
});
...
//cabecera = '<h1><i class="fa fa-info"> </i> Atención </h1>';
									...
									var confirm = alertify.confirm( '¿Esta seguro que quiere tomar el caso?', null, null);
...
$(".btn_search_color").on('click',function(e){
 var request = {
  ...
  var route = '/expedientes?search_onlyMy_exp=on&tipo_busqueda=' + request.tipo_busqueda +'&data='+request.data; 
...
$("#btn_change_doc_exp").on('click',function(e){
  ...
  $("#titulo_modal").text('Cambiando docente') 
     $("#myform_change_docente_exp>#tipo_cambio").val(1);
      ...
      var option = '<option value="' + idnumber + '">' + name.toUpperCase() + ' ' + lastname.toUpperCase() + '</option>'
  ...
  $("#btn_cancel_change_doc_exp").on('click',function(e){
    e.preventDefault();   
   var confirm = alertify.confirm('¿Está seguro de <b>Cancelar</b> la solicitud de cambio?') 
   confirm.set('onok',function(){
...
$("#btn_delete_doc_exp").on('click',function(e){
  e.preventDefault();   
 var confirm = alertify.confirm('¿Está seguro de <b>Eliminar</b> la asignación docente?') 
 confirm.set('onok',function(){
  ...
  $("#btn_accept_change_doc_exp").on('click',function(e){
    e.preventDefault();   
   var confirm = alertify.confirm('¿Está seguro de <b>Aceptar</b> la solicitud de cambio?') 
   confirm.set('onok',function(){
...
$("#myform_change_docente_exp").on('submit',function(e){ 
  var data = $(this).serialize();
  var exp_id = $("#expediente_id").val();
  $("#myform_change_docente_exp input[type='submit]").prop('disabled',true);
  changeDocenteExp(data,exp_id);
...
$("#menu-notification").on('click','#btn_read_notifications',function(e){
  e.preventDefault();
  changeNotifications();
});
...
$("#ct_forcitaest").on('submit','#myformCitarEstudianteEdit',function(e){
...
return false;
 ...
 $("#btn_mod_expfecha_res").on("click",function(e){
  data = {'expfecha_res':$("#expfecha_res").val(),
  'exp_id':$("#expid").val(),
...
$("#myform_exp_edit_cierre_caso select[name=new_expestado]").on("change",function(e){
  console.log(this.value)
  if($(this).val()=='5'){
    $("#lbl_msj_nf").html('El caso se evaluará con notas finales en <b>0</b>').show();
  }else{
...
$("#btn_nueva_autorizacion").on("click",function(){
  ...
  $("#myformCreateAutorizacion button").removeClass('btn-warning').addClass('btn-primary').text('Crear')
        
  $("#mymodalCreateAutorizacion").modal("show")
});

$("#mymodalCreateAutorizacion").on("submit",'#myformCreateAutorizacion',function(e){
...
$("#table_list_autorizaciones").on("click",'.btn_editar_autorizacion',function (e) {

  var id = $(this).attr('data-id');
  editAutorizacion(id)
});

$("#mymodalCreateAutorizacion").on('submit','#myformEditAutorizacion',function (e) {
...
$("#table_list_autorizaciones").on("click",'.btn_eliminar_autorizacion',function (e) {
  ...
  var msj = alertify.confirm("¿Esta seguro de eliminar la autorización?\nNo podrá revertir los cambios");
  msj.set('onok',function(){
...
$("#table_list_autorizaciones").on("click",'.btn_change_estado_autorizacion',function (e) {
  var id = $(this).attr('data-id');  
  if($(this).attr('data-estado')==0){
...
$("#content_list_oficinas").on("click",'.btn_asig_usuarios',function (e) {
  $("#myformUserStore input[name=oficina_id]").val($(this).attr('data-id'))
...
$("#content_list_oficinas").on("click",'.btn_delete_oficina',function (e) {
  var request = {'oficina_id':$(this).attr('data-id')};
  var msj = alertify.confirm("¿Esta seguro de eliminar la oficina, se eliminaran todos los registros asociados.\nNo podrá recuperar los datos!");
    msj.set('onok',function(){
...
$("#myformUserStore").on("blur",'.data_user',function(e) {  
  if(this.value !='' && $("#myformUserStore select[name=tipodoc_id]").val()!=''){
    var request = {
      'idnumber':$(this).val(),
...
$("#myformUserStore select[name=tipodoc_id]").on("change",function(e) {  
  if(this.value !='' && $("#myformUserStore input[name=idnumber]").val()!=''){
    var request = {
      'tipodoc_id':$(this).val(),
...
$("#list_students_oficinas").on("click",'.btn_asig_notas_ext',function(e) {
  $("#myform_asig_nota_ext input[name=estidnumber]").val($(this).attr('data-id'));
  var request = {'idnumber':$(this).attr('data-id'),'oficina_id':$("#oficina_id").val(),'origen':5}
  findNotas(request)
...
$("#myform_asig_nota_ext").on("change",'#chk_change_nota',function (e) {
  ...
  $("#table_list_oficina_usuarios").on('click','.btn_delete_userof',function (e) {
    var request = {'user_id':$(this).attr('data-id'),'oficina_id':$("#oficina_id").val()};
    var msj = alertify.confirm("¿Esta seguro de eliminar el usuario.\nNo podrá recuperar los datos!");
      msj.set('onok',function(){
  ...
  $("#content_oficina_virtual").on("click","#btn_adm_documentos",function(e){    
    $("#myModal_adm_documentos").modal('show');  
  });
 
  $("#content_oficina_virtual").on("click",'#btn_adm_notificaciones',function(e){
    var type_log_id = $(this).attr('data-type');
    $("#myformCreateDocumento input[name=type_log_id]").val(type_log_id);
    $("#myModal_notificacion").modal('show');
    
  });

  $(".content_oficina_virtual").on("click",'#btn_videollamada',function(e){ 
    var type_log_id = $(this).attr('data-type');  
    ...
    $.ajax({
      ...
      success: function (res) {   
          ...
          $('#newtab-stream-cases').attr('href', url+res.room+"?jwt="+res.jwt);
          $('#copy-stream-cases').attr('data-frame', url+res.room+"?jwt="+res.jwt);
          $('#text-stream-cases').val(url+res.room);
          $('#iframe-stream-cases').attr('src', url+res.room+"?jwt="+res.jwt);
  ...
  $("#content_create_notificacion").on("submit",'#myformCreateNotificacion',function (e) {
  ...
  $("#myformAceptSolicitud").on("submit",function(e) {
    var request = $(this).serialize()+
    "&solicitud_id="+$("#solicitud_id").val()+"&type_status_id=156"; 
  ...
  $("#myformDeniedSolicitud").on("submit",function(e) {
    var request = $(this).serialize()+
    "&solicitud_id="+$("#solicitud_id").val()+"&type_status_id=157";   
    updateSolicitud(request,$("#solicitud_id").val()) ;
    e.preventDefault();
  });


  $("#cont_adm_docs").on("submit",'#myformCreateSoliDocumento',function (e) {
    // var request = $(this).serialize()+"&exp_id="+$("#expediente_id").val();
     var request = new FormData($(this)[0]);
  ...
  $("#content_solicitudes_files").on('click','.btn_doc_delete',function (e) {
    var id = $(this).attr('data-id');  
    let request = {'solicitud_id':$("#solicitud_id").val()}
    Swal.fire({
      title: 'Esta seguro de eliminar el documento?',
   ...
   $("#myFormAsigExpe").on('submit',function(e){
    let request   = $("#myFormAsigExpe").serialize()+"&solicitud_id="+$("#solicitud_id").val();
   ...
   $("#btn_new_category").on("click",function(e) {
    ...
    $("#myformCreateCategory button[type=submit]").text('Guardar')
   ...
   $("#btn_new_static_category").on("click",function(e) {
    ...
    $("#aditional_options_table tbody").html('');
    $("#content_aditional_options").hide();
    $("#myformCreateStaticCategory button[type=submit]").text('Guardar')
    .removeClass('btn-warning').addClass('btn-primary');    
    $("#lbl_modal_title").text("Creando categoria");
   ...
   $("#myformCreateStaticCategory select[name=type_data_id]").on("change",function (e) {    
    if($(this).val()=='169' || $(this).val()=='170'){     
      var item = $(".option_row").length;    
      if(item==0){
        addOptionTable(item);            
  ...
  $("#content_aditional_options").on("click",'.btn_delete_option_row',function (e) {
    ...
    $("#myModal_create_category").on('submit','#myformCreateCategory',function(e) {
      var request = $(this).serialize();
      createCategory(request);
      e.preventDefault();
    });

    $("#myModal_create_static_category").on('submit','#myformCreateStaticCategory',function(e) {
      var request = $(this).serialize();
      createStaticCategory(request);
      e.preventDefault();
    });

    $("#myModal_create_category").on("submit",'#myformEditRCategory',function(e){      
    ...
    $("#myModal_create_static_category").on("submit",'#myformEditRStaticCategory',function(e){      
      $("#items_deleted").val(JSON.stringify(items_delete));   
       var request = $(this).serialize(); 
    ...
    $("#content_categories_list").on("click",'.btn_edit_category',function(e){
      let id = $(this).attr('data-id');    
      referenceEdit(id);
      e.preventDefault();
  });

  $("#content_categories_list").on("click",'.btn_edit_con_category',function(e){
    let id = $(this).attr('data-id');    
    referenceConciliacionEdit(id);
    e.preventDefault();
});

  $("#content_aditional_data").on("change",'.data_input_select',function (e) {
    
    let id = $(this).attr('data-id');
    //alert(id)
    let status = $("#option_id-"+id+ " option:selected").attr('data-active_other');
    if(status==1){
    ...
    if($(this).val()!=''){
      if($(this).prop('type')=='select-one'){
        var request = {
          ...
          'name':$(this).attr("data-name"),
          'conciliacion_id':$("#conciliacion_id").val(),
          'reference_data_option_id':$(this).find(':selected').attr('data-option_id')
        }
      }else{
        var request = {
          'value':$(this).val(),
  ...
  $("#myModal_create_document").on("submit",'#myformCreateConciliacionAnexo',function (e) {
   ...
   $("#myModal_create_document").on("submit",'#myformEditConciliacionAnexo',function (e) {
    var request = new FormData($(this)[0]);
    request.append("conciliacion_id", $("#conciliacion_id").val());
  ...
  $("#table_anexos_list").on("click",'.btn_edit_anxcon',function (e) {
    $("#myformCreateConciliacionAnexo").attr('id','myformEditConciliacionAnexo');
    $("#myformEditConciliacionAnexo input[name=file_id]").val($(this).attr('data-id'));
    $("#myformEditConciliacionAnexo input[name=concept]").val($(this).attr('data-concept'));
    $("#myformEditConciliacionAnexo input[name=conciliacion_file]").prop("required",false);
    $("#myformEditConciliacionAnexo button[type=submit]").text("Actualizar");
  ...
  $("#myModal_create_estado").on("submit",'#myformCreateEstado',function (e) {
   ...
   $("#myModal_create_comentario").on("submit",'#myformCreateComentario',function (e) {
    var request = $(this).serialize()+"&conciliacion_id="+$("#conciliacion_id").val();  
    storeConciliacionComentario(request);    
    e.preventDefault(); 
  });

  $("#myModal_create_comentario").on("submit",'#myformEditComentario',function (e) {
    var request = $(this).serialize()+"&conciliacion_id="+$("#conciliacion_id").val();  
  ...
  $("#table_list_comentarios").on('click','.btn_delete_com_con',function(e){
    var request = {
  ...
  $("#table_list_estados").on('click','.btn_edit_est_con',function(e){
   ...
   function editConciliacionComentario(request) {
    ...
    success: function (res) {
          ...
          if(res.compartido==1)$("#myformEditComentario input[name=compartido]").prop('checked',true);
          
          $("#myformEditComentario textarea[name=comentario]").val(res.comentario)
          $("#myformEditComentario button[type=submit]").text("Actualizar");
         $("#myModal_create_comentario .modal-title").text("Actualizado comentario");
          $("#myModal_create_comentario").modal('show')
          $("#wait").hide();
        },
        error: function (xhr, textStatus, thrownError) {
   ...
   function editConciliacionEstado(request) {
    ...
    success: function (res) {
          ...
          $("#myformEditEstado select[name=type_status_id]").val(res.type_status_id);
   ...
   function insertConADValue(request) {
    ...
    success: function (res) {
      ...
      try {
         ...
         }else if(res.render_view || res.render_view==''){                           
  ...
  function  referenceConciliacionEdit(id){
    var route = "/categories/"+id+'/edit';    
              ...
              success: function (res) {
                ...
                if(items_delete.length > 0 ) items_delete.length = 0 ; 
                try {
                    $("#myformCreateStaticCategory").attr('id','myformEditRStaticCategory');
                    $("#myformEditRStaticCategory input[name=id]").val(res.id);
                    $("#myformEditRStaticCategory select[name=type_data_id]").val(res.type_data_id);
                    $("#myformEditRStaticCategory select[name=table]").val(res.table);
                    $("#myformEditRStaticCategory select[name=section]").val(res.section);
                    $("#myformEditRStaticCategory input[name=display_name]").val(res.display_name);
                    $("#myformEditRStaticCategory input[name=name]").val(res.name);
                    $("#myformEditRStaticCategory button[type=submit]").text('Actualizar')
                    .removeClass('btn-primary').addClass('btn-warning');
                    ...
                    if(res.options.length > 0 && (res.type_data_id!=168 && res.type_data_id!=173 && res.type_data_id!=174)){
                        var row = '';
                        res.options.forEach((element,item) => {
                            let checked_ = '';
                            let value = '0';
...
function fillFormEditDoc(res) {  
  ...
  $("#myformEditDocumento textarea[name=description").val(res.caseL.description)
  $("#myformEditDocumento button[type=submit").text('Actualizar').removeClass('btn-primary').addClass('btn-warning');
  ...
  $("#cont_adm_docs #a_tab_create").attr('aria-expanded',true).text('Actualizar');
...
function fillFormEditSoliDoc(res) {  
  ...
  $("#myformEditSoliDocumento").removeClass('form_store').addClass('form_update');  
  $("#myformEditSoliDocumento button[type=submit").text('Actualizar').removeClass('btn-primary').addClass('btn-warning');
  if(res.solicitud.files.length>0){
    $("#myformEditSoliDocumento input[name=id").val(res.solicitud.files[0].id);
    $("#myformEditSoliDocumento input[name=concept").val(res.solicitud.files[0].pivot.concept);
    $(".custom-file-label").text(res.solicitud.files[0].original_name);
    $("#myformEditSoliDocumento input[name=solicitud_file").prop('required',false);
  }else{
    $("#myformEditSoliDocumento input[name=solicitud_file").prop('required',true);
    $(".custom-file-label").text('Sin archivo');
  }
...
function resetFormCDoc(){
  ...
  $("#myformCreateDocumento").removeClass('form_update').addClass('form_store')
  $("#myformCreateDocumento button[type=submit").text('Guardar')
  .removeClass('btn-warning').addClass('btn-primary');
...
function resetFormSoliDoc(){
  ...
  $("#myformCreateSoliDocumento input[name=solicitud_file").prop('required',true);
  $("#myformCreateSoliDocumento").removeClass('form_update').addClass('form_store')
  $("#myformCreateSoliDocumento button[type=submit").text('Guardar')
  .removeClass('btn-warning').addClass('btn-primary');
  $(".custom-file-label").text('');
...
function oficinaUserDelete(request){
  ...
  $.ajax({
    ...
    $.ajax({
      ...
      success: function (res) {   
        if(!res.notas){
          ...
          $("#myform_asig_nota_ext input[name=definitiva]").prop('checked',false).prop('disabled',false);
          $("#myform_asig_nota_ext input[name=ntaconocimiento]").val('').prop('disabled',false);
          $("#myform_asig_nota_ext textarea[name=ntaconcepto]").val('').prop('disabled',false);
          $("#myform_asig_nota_ext input[name=ntaconocimientoid]").val('')
          $("#myform_asig_nota_ext input[name=ntaconceptoid]").val('');          
          $("#myform_asig_nota_ext input[type=submit]").prop('disabled',false).show().val('Guardar');;
          $("#lbl_extevaluador_name").text('');
          $("#myform_asig_nota_ext input[name=typesub]").val('store')
        }else{
          $("#myform_asig_nota_ext input[name=ntaconocimiento]").val(res.notas.nota_conocimiento).prop('disabled',true)
          $("#myform_asig_nota_ext textarea[name=ntaconcepto]").val(res.notas.nota_concepto).prop('disabled',true)
          $("#myform_asig_nota_ext input[name=ntaconocimientoid]").val(res.notas.nota_conocimientoid)
          $("#myform_asig_nota_ext input[name=ntaconceptoid]").val(res.notas.nota_conceptoid)
          $("#myform_asig_nota_ext input[name=typesub]").val('update');         
          $("#myform_asig_nota_ext input[type=submit]").prop('disabled',true).hide();
          $("#lbl_extevaluador_name").text('Evaluado por: '+res.notas.docevname)
         if(res.notas.tipo_id==1){
            $("#myform_asig_nota_ext input[name=definitiva]").prop('checked',true).prop('disabled',true);          
          }else{
            $("#myform_asig_nota_ext input[name=definitiva]").prop('checked',false).prop('disabled',true);
          }
          if(res.notas.can_edit && res.notas.tipo_id==2){
            $("#myform_asig_nota_ext input[type=submit]").prop('disabled',false).show().val('Actualizar'); 
            $("#myform_asig_nota_ext input[name=ntaconocimiento]").prop('disabled',false);
            $("#myform_asig_nota_ext textarea[name=ntaconcepto]").prop('disabled',false);
            $("#myform_asig_nota_ext input[name=definitiva]").prop('disabled',false);                  
          }
        }
   ...
   function oficinaAsigNota(request) {
    var route = "/notasext";
    $.ajax({
      url: route,
      type: 'POST',
   ...
   function findUser(request){
    ...
    $.ajax({
      ...
      success: function (res) {   
         // $("#table_list_autorizaciones tbody").html(res.view)
        if(res.encontrado){
          $("#myformUserStore input[name=id]").val(res.user.id).prop('disabled',false);;
          $("#myformUserStore input[name=idnumber]").val(res.user.idnumber);
          $("#myformUserStore input[name=name]").val(res.user.name).prop('disabled',true);
          $("#myformUserStore input[name=lastname]").val(res.user.lastname).prop('disabled',true);         
          $("#myformUserStore input[name=email]").val(res.user.email).prop('disabled',true);; 
          $("#myformUserStore input[name=tel1").val(res.user.tel1).prop('disabled',true);;
          if(res.user.roles.length>0){
            $("#myformUserStore #lbl_role_name").text(res.user.roles[0].display_name)
            $("#myformUserStore select[name=idrol").val(res.user.roles[0].id).prop('disabled',true).hide();;
          }
        }else{
          $("#myformUserStore input[name=id]").val('').prop('disabled',true);         
          $("#myformUserStore input[name=name]").val('').prop('disabled',false);
          $("#myformUserStore input[name=lastname]").val('').prop('disabled',false);         
          $("#myformUserStore input[name=email]").val('').prop('disabled',false);; 
          $("#myformUserStore input[name=tel1").val('').prop('disabled',false);;
          $("#myformUserStore #lbl_role_name").text('')        
          $("#myformUserStore select[name=idrol").prop('disabled',false).show();;
   ...
   function editAutorizacion(id){
    ...
    $.ajax({
      ...
      success: function (res) {   
           ...
           $("#myformEditAutorizacion input[name=nombre_estudiante]").val(res.nombre_estudiante)
           $("#myformEditAutorizacion input[name=num_identificacion]").val(res.num_identificacion)
           $("#myformEditAutorizacion input[name=doc_expedicion]").val(res.doc_expedicion)
           $("#myformEditAutorizacion input[name=num_carne]").val(res.num_carne)
           $("#myformEditAutorizacion input[name=calidad_de]").val(res.calidad_de)
           $("#myformEditAutorizacion input[name=tipo_proceso]").val(res.tipo_proceso)
           $("#myformEditAutorizacion input[name=num_radicado]").val(res.num_radicado)
           $("#myformEditAutorizacion input[name=juzgado]").val(res.juzgado)
           $("#myformEditAutorizacion select[name=genero]").val(res.genero)
           $("#myformEditAutorizacion button").removeClass('btn-primary').addClass('btn-warning').text('Actualizar')
...
function createBiblioteca(){
 ...
 // alert(errors)
  ...
  if( input.files && input.files.length >= 1 ){
       ...
       // var token = $("#token").val();
        var formData= new FormData(document.getElementById('myformCreateBiblioteca'));
        $.ajax({ 
        url: route,
        headers: { 'X-CSRF-TOKEN' : token },
        type:'POST',
...
function updateBiblioteca(){
        ...
        //var token = $("#token").val();
        var formData= new FormData(document.getElementById('myformUpdateBiblioteca'));
...
////console.log(errors)
  if (errors<=0) {
   ...
   apl_shared = $("#apl_shared").val();
    ...
    type:'POST',
    datatype: 'json',
    data:{'comentario':comentario,'expid':expid,'apl_shared':apl_shared},
    cache: false,
     beforeSend: function(xhr){
      xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));
      $("#wait").css("display", "block");   
...
function storeEstadoCaso(){
  ...
  if (errors.length>0) {
    ...
    $("#msg-danger label").append('Recuerda que antes de enviar esta solicitud debes diligenciar los datos del caso y <b>GUARDAR.</b><hr>');
    ...
    type:'POST',
    datatype: 'json',
    data:{'hechos':hechos,'exp_resp_est':exp_resp_est,'new_expestado':new_expestado,'motivo_estado':motivo_estado,'comentario':comentario,'expid':expid},
    cache: false,
    beforeSend: function(xhr){
      $("#wait").css("display", "block");  
      xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));
...
function searchEstadoCaso(id){
   ...
   // var token = $("#token").val();  
    ...
    success:function(res){
      ...
      $("#nombre_usuario_details").text(res.user.name+' '+res.user.lastname);
  ...
  var route = "/asesorias/change/shared";
   $.ajax({ 
    ...
    success:function(res){
      ////console.log(res);
      if (res.apl_shared) {
        $("#switch_edit"+id).removeClass('switch-off').addClass('switch-on');
      }else if(!res.apl_shared){
        $("#switch_edit"+id).removeClass('switch-on').addClass('switch-off');
      }
...
function reasigCaso(){
 ...
 // var token = $("#token").val();  
  var route = '/expedientes/reasigcaso'; 
    ...
    type:'POST',
    datatype: 'json',
    data:{'expid':expid,'new_user_id':new_user_id,'anotacion':anotacion,'periodo_id':periodo_id,'motivo_asig_id':motivo_asig_id},
    cache: false,
    beforeSend: function(xhr){
      $("#wait").css("display", "block");   
      xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));
...
function abrirModalDocentes(res,option){
  ...
  for (var i = res.length - 1; i >= 0; i--) {
    if ($("#doc_id_number").val() != res[i].idnumber) options += '<option value="' + res[i].idnumber + '">' + res[i].full_name.toUpperCase() + '</option>';
  }
...
var row = '';
   ...
   row +='<td>';
   row += '<select id="dataSelectAct'+index+'" required name="numberestact_id[]" class="form-control selectpicker disabled-fun4 dangerous" data-live-search="true" onchange="comparateData('+index+')">';
   row +='<option value="">Seleccione...</option>';
   for (var i = res.length - 1; i >= 0; i--) {
   row +='<option value="'+res[i].idnumber+'">'+res[i].full_name+'</option>';
 }
   row += '</select>';
   row +='</td>';
  row +='<td>';
  row +='</td>';
  row +='<td>';
   row += '<select id="dataSelectNew'+index+'" required name="numberestnew_id[]" class="form-control selectpicker disabled-fun4 dangerous" data-live-search="true" onchange="comparateData('+index+')">';
    row +='<option value="">Seleccione...</option>';
   for (var i = res.length - 1; i >= 0; i--) {
   row +='<option value="'+res[i].idnumber+'">'+res[i].full_name+'</option>';
    }
   ...
   row +='<a class="btn btn-danger" onclick="deleteRow('+index+')"><i class="fa fa-minus-circle"></> Eliminar</a>';
...
function llenarFormAuditoria(res,data,expediente_actual){
  ...
  $("#label_userAuditoria").text(res.auditoria.usuario.name+' '+res.auditoria.usuario.lastname);
  ...
  if (data.exphechos!=null) {
    $("#label_exphechosAuditoria").html(data.exphechos.replace(/\n/g, "<br />"));
  }
  if (data.exprtaest!=null) {
    $("#label_exprtaestAuditoria").html(data.exprtaest.replace(/\n/g, "<br />"));
  }
 ...
 $("#label_expidActual").html(expediente_actual.expid);
  ...
  $("#label_expramaderecho_idActual").html(expediente_actual.expramaderecho_id);
  ...
  if (expediente_actual.exphechos!=null) {
    $("#label_exphechosActual").html(expediente_actual.exphechos.replace(/\n/g, "<br />"));
  
  }
  if (expediente_actual.exprtaest!=null) {
    $("#label_exprtaestActual").html(expediente_actual.exprtaest.replace(/\n/g, "<br />"));  
  }
  ...
  $("#label_exppersondemandanteActual").html(expediente_actual.exppersondemandante);
  $("#label_exppersondemandadaActual").html(expediente_actual.exppersondemandada);
...
table.html('');
  ...
  for (var i = res.length - 1; i >= 0; i--) {
    ...
    row +='</td>';
    row +='<td>';
    row +='<a class="btn btn-danger btn_deleteasignacionaoc" id="deleteasig_'+res[i].id+'">Quitar Asignación</a>';
    row +='</td>';
    row +='</tr>'; 
  }
table.append(row);
...
function actualizarHorario(id){
  ...
  if (errors.length<=0) {
    ...
    type:'PUT',
    datatype: 'json',
    data:{'horas_a':horas_a,'horas_b':horas_b,'num_max_est':num_max_est},
    cache: false,
    beforeSend: function(xhr){
      $("#wait").css("display", "block");   
      xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));
...
function updateAsigTurno(turno_id){
  ...
  var route = '/turnos/'+turno_id;   
      $.ajax({ 
        ...
        type:'PUT',
        datatype: 'json',
        data:{'trnid_dia':trnid_dia,'trnid_oficina':trnid_oficina,'estudiante_id':estudiante_id,'color_id':color_id,'horario_id':horario_id,'cursando_id':cursando_id},
        cache: false,
        beforeSend: function(xhr){
          $("#wait").css("display", "block");   
          xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));
...
function asigTurnoEst(){
  ...
  var route = '/turnos';   
      $.ajax({ 
        ...
        type:'POST',
        datatype: 'json',
        data:{'trnid_oficina':trnid_oficina,'trnid_dia':trnid_dia,'idnumber':idnumber,'color_id':color_id,'horario_id':horario_id,'cursando_id':cursando_id},
        cache: false,
        beforeSend: function(xhr){
          $("#wait").css("display", "block");   
          xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));
...
for (var i = res.length - 1; i >= 0; i--) {
  ...
  row +='</td>';
  row +='<td>';
  row +='<a class="btn btn-success btn_asgicasest" id="'+res[i].idnumber+'"> Detalles</a>';
  row +='</td>';
  row +='</tr>';
}
 //  ////console.log(row)
  ...
  if (errors<=0) { 
     var route = '/expedientes/buscarexpasig';   
      $.ajax({ 
        ...
        type:'POST',
        datatype: 'json',
        data:{'fech_desde':fech_desde,'fech_hasta':fech_hasta,'idnumber':idnumber},
        cache: false,
        beforeSend: function(xhr){
          //$("#wait").css("display", "block");   
          xhr.setRequestHeader('X-CSRF-TOKEN', $("#token").attr('content'));
...
for (var i = res.length - 1; i >= 0; i--) {
  row +='<tr>';
    ...
    row +='</td>';
    row +='<td>';
    row +='<a href="/expedientes/'+res[i].id+'/edit" class="btn btn-primary" id="'+res[i].idnumber+'"> Ir al Caso</a>';
    row +='</td>';
  row +='</tr>';
}
 //  ////console.log(row)
  ...
  var route = '/segmentos/'+id;   
    ...
    success:function(res){
     // data = $.parseJSON(res.data)
      ////console.log(res); 
      //$("#myform_edit_periodo").loadJSON(res)
      $("#myform_edit_segmento input[name='segnombre']").val(res.segnombre);  
      $("#myform_edit_segmento input[name='fecha_inicio']").val(res.fecha_inicio);  
     $("#myform_edit_segmento input[name='fecha_fin']").val(res.fecha_fin);  
   ...
   // data:id,
    ...
    success:function(res){
      if(res.saved){    
       ...
       // console.log(res)
        $("#msg-danger label").html('');
        $("#msg-danger label").append('Recuerda que debe haber un final de corte activo');
        $("#msg-danger").show(); 
      }
 ...
 var route = '/notas/'+tbl_id+'/edit';
    ...
    success:function(res){
     // console.log(res);
      ...
      $("#myform_update_notas #nota_conocimiento").val(res.nota_conocimiento);
      $("#myform_update_notas #nota_conocimientoid").val(res.nota_conocimientoid);
      ...
      $("#myform_update_notas #nota_aplicacionid").val(res.nota_aplicacionid); 
      ...
      if (res.encontrado){
        ...
        if(res.can_edit){  
          if(origen==1 && $("#expestado_id").val()=='4') {           
            ...
            $("#btn_tipo_update").attr('id','btn_tipo_nota_update'); 
          }
      ...
      if(origen==3) { 
        $("#myModal_edit_notas .fil_nt_co input[type='text']").attr('type','hidden')
        .prop('disabled',true);
        $("#myModal_edit_notas .fil_nt_co").hide();
       // hideElement('btn_delete_notas');
      }else{
        $("#myModal_edit_notas .fil_nt_co input[type='hidden']").attr('type','text')
        .prop('disabled',false);
...
function searchSegmentos(periodo_id,type){
  var route = '/periodos/buscar/segmentos/'+periodo_id;
    ...
    success:function(res){
     ...
     if(type=='gen') $("#myFormNotasSearch select[name=segmento_id]").html(option);
     if(type=='ind') $("#myFormNotasSearchInd select[name=segmento_id]").html(option)
     // $(" #segmento_id").html(option);
      $("#wait").css("display", "none"); 
...
function myTimer() {
    ...
    var cadena =''+' '+mes+'/'+dia+'/'+anio+' '+d.toLocaleTimeString();
...
function reloadSearchExp() {
  ...
  if ($('#tipo_busqueda').val() !== 'undefined' && $('#tipo_busqueda').val() !== '') {
...
function changeSelectSearchExp(value) {
  ...
  switch(value) {
    ...
    case 'idnumber_doc':
      ...
      $('#select_data_consultantes').attr('data-select-origen','docente');
    ...
    $('#select_data_consultantes').attr('data-select-origen','consultante');
